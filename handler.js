...
module.exports = {
    async handler(chatUpdate) {
        ...
        try {
            ...
            const isROwner = [conn.decodeJid(global.conn.user.id), ...global.owner.map(([number, is, isDeveloper]) => number)].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    try {
                        ...
                        if (e) {
                            ...
                            if (e.name) for (let [jid] of global.owner.filter(([number, is, isDeveloper]) => isDeveloper && number)) {
